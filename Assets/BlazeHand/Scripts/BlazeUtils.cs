...
public static class BlazeUtils
{
    ...
    public static ((FunctionalTensor, FunctionalTensor, FunctionalTensor),(FunctionalTensor, FunctionalTensor, FunctionalTensor)) ArgMaxFiltering(FunctionalTensor rawBoxes, FunctionalTensor rawScores)
    {
        var detectionScores = ScoreFiltering(rawScores, 100f); // (1, 2016, 1)  
        //var bestScoreIndex = Functional.ArgMax(rawScores, 1).Squeeze();

        var topKValues = Functional.TopK(rawScores, 10, 1);

        var topKIndices = topKValues[1];


        //use index select to unpack the whoe topKIndices[0,..] score then return selected boxes as a whole
        //REAAAAAAAAAAADDDDDDDDDDDD THISSSSSSSSSSSSSSS
        var bestScoreIndex = topKIndices[0,0].Squeeze();
        var secondBestScore = topKIndices[0,9].Squeeze();

        Debug.Log(bestScoreIndex);


        var selectedBox1 = Functional.IndexSelect(rawBoxes, 1, bestScoreIndex).Unsqueeze(0); // (1, 1, 16)
        var selectedScore1 = Functional.IndexSelect(detectionScores, 1, bestScoreIndex).Unsqueeze(0); // (1, 1, 1)

        var selectedBox2 = Functional.IndexSelect(rawBoxes, 1, secondBestScore).Unsqueeze(0); // (1, 1, 16)
        var selectedScore2 = Functional.IndexSelect(detectionScores, 1, secondBestScore).Unsqueeze(0); // (1, 1, 1)

        return ((bestScoreIndex, selectedScore1, selectedBox1),(secondBestScore, selectedScore2, selectedBox2));
    }

    public static (FunctionalTensor, FunctionalTensor, FunctionalTensor) Top10(FunctionalTensor rawBoxes, FunctionalTensor rawScores)
    {
        var detectionScores = ScoreFiltering(rawScores, 100f); // (1, 2016, 1)  
        //var bestScoreIndex = Functional.ArgMax(rawScores, 1).Squeeze();

        var topKValues = Functional.TopK(rawScores, 20, 1);
        ...
        //use index select to unpack the whoe topKIndices[0,..] score then return selected boxes as a whole
        //REAAAAAAAAAAADDDDDDDDDDDD THISSSSSSSSSSSSSSS
        var bestScoreIndex = topKIndices[0, ..].Squeeze();
        ...
        return (bestScoreIndex, selectedScore1, selectedBox1);
    }
